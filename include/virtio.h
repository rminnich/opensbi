...
inline struct virtio_dev_priv *dev_get_uclass_priv(struct udevice *vdev)
{
	return &(vdev->priv);
}

inline u32 dev_get_flags(struct udevice *vdev)
{
	return 0;
}
...
{
	sbi_panic("better implement __virtio_set_bit");
#if 0
	struct virtio_dev_priv *uc_priv = dev_get_uclass_priv(udev);
...
{
	sbi_panic("better implement __virtio_clear_bit");
#if 0
	struct virtio_dev_priv *uc_priv = dev_get_uclass_priv(udev);
				 ...
				 unsigned int offset)
{
	...
	virtio_get_config(vdev, offset, &ret, sizeof(ret));
	return virtio16_to_cpu(vdev, (__virtio16)ret);
}

static inline void virtio_cwrite16(struct udevice *vdev,
				   unsigned int offset, u16 val)
{
	val = (u16)cpu_to_virtio16(vdev, val);
	virtio_set_config(vdev, offset, &val, sizeof(val));
}
				 ...
				 unsigned int offset)
{
	...
	virtio_get_config(vdev, offset, &ret, sizeof(ret));
	return virtio32_to_cpu(vdev, (__virtio32)ret);
}

static inline void virtio_cwrite32(struct udevice *vdev,
				   unsigned int offset, u32 val)
{
	val = (u32)cpu_to_virtio32(vdev, val);
	virtio_set_config(vdev, offset, &val, sizeof(val));
}
				 ...
				 unsigned int offset)
{
	...
	return virtio64_to_cpu(vdev, (__virtio64)ret);
}

static inline void virtio_cwrite64(struct udevice *vdev,
				   unsigned int offset, u64 val)
{
	val = (u64)cpu_to_virtio64(vdev, val);
	virtio_set_config(vdev, offset, &val, sizeof(val));
}
